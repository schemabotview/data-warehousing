...
//
// `base` is `/data-warehousing/` for the production BUILD only (the app deploys under
// graphl.in/data-warehousing/ as a concept app in the GraphL catalog, so built asset URLs must be
...
export default defineConfig(({ command }) => ({
  base: command === 'build' ? '/data-warehousing/' : '/',
  plugins: [react()],
  resolve: {

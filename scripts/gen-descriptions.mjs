...
// host: SITE default graphl.in, APP_PATH default the vite build base (/data-warehousing). Deep link is
...
const APP_PATH = (process.env.APP_PATH ?? '/data-warehousing').replace(/\/$/, '')
const HASHTAGS =
  '#DataWarehouse #DataWarehousing #DataModeling #DimensionalModeling #StarSchema #Kimball #ETL #DataEngineering #TechEducation'

// Curated PUBLISH titles (scripts/titles.json), keyed by course id — the search-facing name a course
// carries on YouTube ("Star & Snowflake Schemas"), deliberately distinct from the registry's narrative
...
function compose({ course, chapters, series }) {
  ...
  // concept ("Data Warehousing Basics · Data Warehousing" stammers; the publish title is the whole name).

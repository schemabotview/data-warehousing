...
const CONCEPT = process.env.CONCEPT ?? 'Data Warehousing'
// The right-side panel gradient (brand block), anchored on DW blue --brand #5b8cff (src/index.css)
// — the same accent as the DATA WAREHOUSING eyebrow, the slide titles and every `service` node, so the
// panel and the scene beside it are one palette. Reads well against the Zed-slate scene; override with
// --panel. Blue is the one free hue in the catalog, so the concept stays apart from aws/spark/
// databricks (warm), linux (gold) and sql (teal) at thumbnail size where hue is all that survives.
const DEFAULT_PANEL_BG = 'radial-gradient(118% 104% at 70% 34%, #5b8cff 0%, #2a4fb8 44%, #0b1330 100%)'

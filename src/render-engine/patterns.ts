...
export const PATTERNS: Record<PatternKey, PatternStyle> = {
  service: { icon: Server, color: '#5b8cff', bg: '#141c2e' }, // DW brand blue (matches --brand)
  storage: { icon: Database, color: '#37b877', bg: '#122419' },
  // NOTE: `network` is a near-twin of the DW brand blue above, so the two are not safely
  // distinguishable side by side. A warehouse map has no networking role anyway (sources,
  // pipelines, storage, marts and BI all map to service / storage / user / external), so DW
  // scenes should NOT use `network`. If one ever genuinely needs it, repoint this hue first.
